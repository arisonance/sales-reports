...
export default function DirectorEditPage({ params }: Props) {
  const { id } = use(params)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isNew ? '/api/directors' : `/api/directors/${id}`
      ...
      router.push('/admin/manage/directors')
    } catch (err) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-foreground uppercase tracking-wide">
              {isNew ? 'Add Director' : 'Edit Director'}
            </h1>
          </div>
          <Link
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., jsmith@sonance.com"
                required
                className="w-full px-4 py-3 border-2 border-card-border rounded-lg bg-input-bg text-foreground focus:ring-2 focus:ring-sonance-blue focus:border-sonance-blue"
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {saving ? 'Saving...' : isNew ? 'Create Director' : 'Save Changes'}
              </button>
              <Link
